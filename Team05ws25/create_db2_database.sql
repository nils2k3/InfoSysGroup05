...
CREATE TABLE PLANNING_TOOL.OFFERING_ASSIGNMENT (
  ...
  OA_SHARED_GROUP_ID VARCHAR(255),
  OA_ROLE VARCHAR(50),
  PRIMARY KEY (OA_ID),
...
SELECT
  ...
  T.T_NAME,
  T.T_LASTNAME,
  SUM(W.WORKLOAD_HOURS) AS ACTUAL_HOURS,
  W.SP_TERM
FROM PLANNING_TOOL.TEACHER T
JOIN (
  SELECT
    OA.FK_T_ID AS T_ID,
    SP.SP_TERM AS SP_TERM,
    COALESCE(SUM(OA.OA_ASSIGNED_HOURS), 0) AS WORKLOAD_HOURS
  FROM PLANNING_TOOL.OFFERING_ASSIGNMENT OA
  JOIN PLANNING_TOOL.OFFERING O ON O.O_ID = OA.FK_O_ID
  JOIN PLANNING_TOOL.SEMESTER_PLANNING SP ON SP.SP_ID = O.FK_SP_ID
  GROUP BY OA.FK_T_ID, SP.SP_TERM
  UNION ALL
  SELECT
    PA.FK_P_ID AS T_ID,
    SP.SP_TERM AS SP_TERM,
    COALESCE(SUM(PA.PA_REDUCTION_HOURS), 0) AS WORKLOAD_HOURS
  FROM PLANNING_TOOL.POSITION_ASSIGNMENT PA
  JOIN PLANNING_TOOL.POSITION_SEMESTER PS ON PS.PS_ID = PA.FK_PS_ID
  JOIN PLANNING_TOOL.SEMESTER_PLANNING SP ON SP.SP_ID = PS.FK_SP_ID
  GROUP BY PA.FK_P_ID, SP.SP_TERM
) W ON W.T_ID = T.T_ID
GROUP BY T.T_ID, T.T_NAME, T.T_LASTNAME, W.SP_TERM;
...
WITH OA_DEDUP AS (
  SELECT
    ...
    SP.SP_TERM,
    CASE
      WHEN OA.OA_SHARED_GROUP_ID IS NULL THEN OA.OA_ASSIGNED_HOURS
      WHEN ROW_NUMBER() OVER (
        PARTITION BY T.T_ID, SP.SP_TERM, OA.OA_SHARED_GROUP_ID
        ORDER BY OA.OA_ID
      ) = 1 THEN OA.OA_ASSIGNED_HOURS
      ELSE 0
...
TERM_SUM AS (
  SELECT
    COALESCE(OA.T_ID, PA.T_ID) AS T_ID,
    COALESCE(OA.SP_TERM, PA.SP_TERM) AS SP_TERM,
    COALESCE(OA.ASSIGNED_HOURS, 0) AS ASSIGNED_HOURS,
    COALESCE(PA.REDUCTION_HOURS, 0) AS REDUCTION_HOURS
  FROM OA_SUM OA
  FULL OUTER JOIN PA_SUM PA
    ON OA.T_ID = PA.T_ID AND OA.SP_TERM = PA.SP_TERM
)
SELECT
  T.T_ID,
  T.T_NAME,
  T.T_LASTNAME,
  COALESCE(TERM_SUM.ASSIGNED_HOURS, 0) AS ASSIGNED_HOURS,
  COALESCE(TERM_SUM.REDUCTION_HOURS, 0) AS REDUCTION_HOURS,
  COALESCE(TERM_SUM.ASSIGNED_HOURS, 0) + COALESCE(TERM_SUM.REDUCTION_HOURS, 0) AS TOTAL_WORKLOAD,
  TERM_SUM.SP_TERM
FROM PLANNING_TOOL.TEACHER T
LEFT JOIN TERM_SUM ON TERM_SUM.T_ID = T.T_ID;
...
-- View: offering assignments per study program and semester
CREATE VIEW PLANNING_TOOL.STUDY_PROGRAM_OFFERING_ASSIGNMENTS AS
SELECT
  OA.OA_ID,
  OA.FK_O_ID,
  OA.FK_T_ID,
  OA.OA_ASSIGNED_HOURS,
  OA.OA_ACTUAL_HOURS,
  OA.OA_ROLE,
  S.S_ID,
  S.S_NAME,
...
-- View: study program aggregates by semester and term
CREATE VIEW PLANNING_TOOL.STUDY_PROGRAM_TERM_SUMMARY AS
SELECT
  S.FK_ST_NAME AS ST_NAME,
  S.S_SEMESTER,
  SP.SP_TERM,
  COUNT(OA.OA_ASSIGNED_HOURS) AS CNTLEC_COUNT,
  COALESCE(SUM(S.S_STUPO_HOURS), 0) AS SUM_NUMCURR,
  COALESCE(SUM(OA.OA_ACTUAL_HOURS), 0) AS SUM_NUMSCHD,
  COALESCE(SUM(OA.OA_ASSIGNED_HOURS), 0) AS SUM_CNTLEC
FROM PLANNING_TOOL.OFFERING_ASSIGNMENT OA
JOIN PLANNING_TOOL.OFFERING O ON O.O_ID = OA.FK_O_ID
JOIN PLANNING_TOOL.SUBJECT S ON S.S_ID = O.FK_S_ID
JOIN PLANNING_TOOL.SEMESTER_PLANNING SP ON SP.SP_ID = O.FK_SP_ID
GROUP BY S.FK_ST_NAME, S.S_SEMESTER, SP.SP_TERM;

-- View: workload reduction per professor and semester
CREATE VIEW PLANNING_TOOL.PROFESSOR_WORKLOAD_REDUCTION AS
SELECT
  T.T_ID,
  T.T_NAME,
  T.T_LASTNAME,
  COALESCE(SUM(PA.PA_REDUCTION_HOURS), 0) AS REDUCTION_HOURS,
  SP.SP_TERM
FROM PLANNING_TOOL.PROFESSOR P
JOIN PLANNING_TOOL.TEACHER T ON T.T_ID = P.T_ID
JOIN PLANNING_TOOL.POSITION_ASSIGNMENT PA ON PA.FK_P_ID = T.T_ID
JOIN PLANNING_TOOL.POSITION_SEMESTER PS ON PS.PS_ID = PA.FK_PS_ID
JOIN PLANNING_TOOL.SEMESTER_PLANNING SP ON SP.SP_ID = PS.FK_SP_ID
