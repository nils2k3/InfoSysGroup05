...
SELECT
  ...
  X.T_NAME,
  X.T_LASTNAME,
  COALESCE(SUM(X.ACTUAL_HOURS), 0) AS ACTUAL_HOURS,
  X.SP_TERM
FROM (
  SELECT
    T.T_ID,
    ...
    SP.SP_TERM,
    CASE
      WHEN OA.OA_SHARED_GROUP_ID IS NULL THEN OA.OA_ACTUAL_HOURS
      WHEN ROW_NUMBER() OVER (
        PARTITION BY T.T_ID, SP.SP_TERM, OA.OA_SHARED_GROUP_ID
        ORDER BY OA.OA_ID
      ) = 1 THEN OA.OA_ACTUAL_HOURS
      ELSE 0
    END AS ACTUAL_HOURS
...
GROUP BY X.T_ID, X.T_NAME, X.T_LASTNAME, X.SP_TERM;
