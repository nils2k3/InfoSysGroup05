...
CREATE VIEW public.TEACHER_ACTUAL_WORKLOAD AS
SELECT
  T.T_ID AS T_ID,
  T.T_NAME AS T_NAME,
  T.T_LASTNAME AS T_LASTNAME,
  COALESCE(SUM(OA.OA_ACTUAL_HOURS), 0) AS ACTUAL_HOURS,
  SP.SP_TERM AS SP_TERM
FROM public.TEACHER T
JOIN public.OFFERING_ASSIGNMENT OA ON OA.FK_T_ID = T.T_ID
JOIN public.OFFERING O ON O.O_ID = OA.FK_O_ID
JOIN public.SEMESTER_PLANNING SP ON SP.SP_ID = O.FK_SP_ID
GROUP BY T.T_ID, T.T_NAME, T.T_LASTNAME, SP.SP_TERM
;

-- View: workload reduction per professor and semester
CREATE VIEW public.PROFESSOR_WORKLOAD_REDUCTION AS
SELECT
  T.T_ID AS T_ID,
  T.T_NAME AS T_NAME,
  T.T_LASTNAME AS T_LASTNAME,
...
JOIN public.TEACHER T ON T.T_ID = P.T_ID
JOIN public.POSITION_ASSIGNMENT PA ON PA.FK_P_ID = T.T_ID
JOIN public.POSITION_SEMESTER PS ON PS.PS_ID = PA.FK_PS_ID
JOIN public.SEMESTER_PLANNING SP ON SP.SP_ID = PS.FK_SP_ID
GROUP BY T.T_ID, T.T_NAME, T.T_LASTNAME, SP.SP_TERM
;

-- View: estimated workload per professor and semester
CREATE VIEW public.PROFESSOR_ESTIMATED_WORKLOAD AS
SELECT
  T.T_ID AS T_ID,
  T.T_NAME AS T_NAME,
  T.T_LASTNAME AS T_LASTNAME,
  ...
  SP.SP_TERM AS SP_TERM
FROM public.TEACHER T
LEFT JOIN public.OFFERING_ASSIGNMENT OA ON OA.FK_T_ID = T.T_ID
LEFT JOIN public.OFFERING O ON O.O_ID = OA.FK_O_ID
LEFT JOIN public.POSITION_ASSIGNMENT PA ON PA.FK_P_ID = T.T_ID
LEFT JOIN public.POSITION_SEMESTER PS ON PS.PS_ID = PA.FK_PS_ID
LEFT JOIN public.SEMESTER_PLANNING SP ON SP.SP_ID = COALESCE(O.FK_SP_ID, PS.FK_SP_ID)
GROUP BY T.T_ID, T.T_NAME, T.T_LASTNAME, SP.SP_TERM
;

-- View: offered courses per study program and semester
CREATE VIEW public.STUDY_PROGRAM_OFFERED_COURSES AS
SELECT
  S.S_ID AS S_ID,
  S.S_NAME AS S_NAME,
  S.S_SEMESTER AS S_SEMESTER,
  SP.ST_NAME AS ST_NAME,
  O.FK_SP_ID AS FK_SP_ID,
  SEM.SP_TERM AS SP_TERM
FROM public.OFFERING O
JOIN public.SUBJECT S ON S.S_ID = O.FK_S_ID
LEFT JOIN public.STUDY_PROGRAM SP ON SP.ST_NAME = S.FK_ST_NAME
LEFT JOIN public.SEMESTER_PLANNING SEM ON SEM.SP_ID = O.FK_SP_ID
;

-- View: offering assignments per study program and semester
CREATE VIEW public.STUDY_PROGRAM_OFFERING_ASSIGNMENTS AS
SELECT
  OA.OA_ID AS OA_ID,
  OA.FK_O_ID AS FK_O_ID,
  OA.FK_T_ID AS FK_T_ID,
  OA.OA_ASSIGNED_HOURS AS OA_ASSIGNED_HOURS,
  OA.OA_ACTUAL_HOURS AS OA_ACTUAL_HOURS,
  OA.OA_ROLE AS OA_ROLE,
  S.S_ID AS S_ID,
  S.S_NAME AS S_NAME,
  S.S_SEMESTER AS S_SEMESTER,
  SP.ST_NAME AS ST_NAME,
  O.FK_SP_ID AS FK_SP_ID,
  SEM.SP_TERM AS SP_TERM
FROM public.OFFERING_ASSIGNMENT OA
JOIN public.OFFERING O ON O.O_ID = OA.FK_O_ID
JOIN public.SUBJECT S ON S.S_ID = O.FK_S_ID
LEFT JOIN public.STUDY_PROGRAM SP ON SP.ST_NAME = S.FK_ST_NAME
LEFT JOIN public.SEMESTER_PLANNING SEM ON SEM.SP_ID = O.FK_SP_ID
